#!/usr/bin/env python3
"""
Test the fixed CRC algorithm against Frame 69 packet
"""

import sys
sys.path.insert(0, '/home/user/XKOP_UG405')

# We need to import the fixed functions from app.py
# But app.py has Flask dependencies, so let's just copy the fixed algorithm here
...
def xkop_crc_check(packet: bytes) -> bool:
    """Official CRC check - processes full packet including CRC"""
    crc1 = 0
    crc0 = 0
...
error_log = bytes.fromhex("CA35020000000000000000000000009BA0")

print("="*80)
print("TESTING FIXED CRC ALGORITHM")
print("="*80)

print("\n" + "="*80)
print("Frame 69 Packet")
print("="*80)
print(f"Full packet: {frame69.hex().upper()}")
print(f"Data (first 15 bytes): {frame69[:15].hex().upper()}")
print(f"CRC bytes: {frame69[15:17].hex().upper()}")

# Calculate CRC
calc_crc = xkop_crc(frame69[:15])
print(f"\nCalculated CRC: {calc_crc.hex().upper()}")
print(f"Received CRC:   {frame69[15:17].hex().upper()}")
print(f"Match: {calc_crc == frame69[15:17]}")

# Validate full packet
valid = xkop_crc_check(frame69)
print(f"\nFull packet validation: {'✓ VALID' if valid else '✗ INVALID'}")

print("\n" + "="*80)
print("Error Log Packet")
print("="*80)
print(f"Full packet: {error_log.hex().upper()}")
print(f"Data (first 15 bytes): {error_log[:15].hex().upper()}")
print(f"CRC bytes: {error_log[15:17].hex().upper()}")

# Calculate CRC
calc_crc = xkop_crc(error_log[:15])
print(f"\nCalculated CRC: {calc_crc.hex().upper()}")
print(f"Received CRC:   {error_log[15:17].hex().upper()}")
print(f"Match: {calc_crc == error_log[15:17]}")

# Validate full packet
valid = xkop_crc_check(error_log)
print(f"\nFull packet validation: {'✓ VALID' if valid else '✗ INVALID'}")
...
if xkop_crc_check(frame69) and xkop_crc_check(error_log):
    print("\n✓ SUCCESS! Both packets are now VALID with the official CRC algorithm!")
    print("\nThe fix is correct. The app.py parser will now accept packets from the")
    print("real UG405 controller.")
else:
    print("\n✗ FAILURE! Packets still invalid.")
    print("There may be an additional issue to investigate.")

#!/usr/bin/env python3
"""
Test the corrected CRC implementation with real controller packets
"""
...
def xkop_crc(data: bytes) -> bytes:
    """
    CORRECTED CRC algorithm: Initialize with header bytes
    """
    crc1 = XKOP_HDR1  # 0xCA
    crc0 = XKOP_HDR2  # 0x35
...
# Real controller packets
test_packets = [
    ("Alive message", bytes.fromhex("CA 35 02 00 00 00 00 00 00 00 00 00 00 00 00 9B A0")),
    ("Data message 1", bytes.fromhex("CA 35 00 00 00 01 01 00 00 FF 00 00 FF 00 00 99 7C")),
    ("Data message 2", bytes.fromhex("CA 35 00 03 00 01 FF 00 00 FF 00 00 FF 00 00 D6 90")),
    ("Data message 3", bytes.fromhex("CA 35 00 01 00 01 FF 00 00 FF 00 00 FF 00 00 AF 4D")),
    ("Data message 4", bytes.fromhex("CA 35 00 00 00 00 01 00 00 02 00 00 03 00 00 6F 31")),
]

print("=" * 80)
print("FINAL CRC FIX VERIFICATION")
print("=" * 80)
print("\nCORRECT ALGORITHM DISCOVERED:")
print("  Initialize: crc1 = 0xCA (header byte 1)")
print("              crc0 = 0x35 (header byte 2)")
print("  Process:    All 15 bytes (including header)")
print("  Result:     2-byte CRC appended to packet")
print("=" * 80)

all_pass = True

print("\nTest 1: CRC Calculation on Real Controller Packets")
print("-" * 80)
for desc, full_packet in test_packets:
    data = full_packet[:15]
    expected_crc = full_packet[15:17]
    calc_crc = xkop_crc(data)

    match = calc_crc == expected_crc
    all_pass = all_pass and match

    status = "✓ PASS" if match else "✗ FAIL"
    print(f"{status} | {desc:20} | Expected: {expected_crc.hex().upper()} | Calculated: {calc_crc.hex().upper()}")

print("\nTest 2: Full Packet CRC Validation")
print("-" * 80)
for desc, full_packet in test_packets:
    is_valid = xkop_crc_check(full_packet)
...
if all_pass:
    ...
    print("=" * 80)
    print("\nThe CRC algorithm is now CORRECT:")
    print("  ✓ Matches all real controller packets")
    print("  ✓ Full packet validation works")
    print("  ✓ Error detection is enabled")
    print("\napp.py has been updated with the corrected algorithm.")
else:

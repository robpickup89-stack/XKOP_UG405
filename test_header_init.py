#!/usr/bin/env python3
"""
Test CRC with header bytes as init values
User's discovery: crc1=0xCA, crc0=0x35 (the header bytes!)
"""
...
def xkop_crc_with_header_init(data: bytes) -> bytes:
    """
    CRC calculation with header bytes as initial values
    crc1 = 0xCA (XKOP_HDR1)
    crc0 = 0x35 (XKOP_HDR2)
    """
    crc1 = 0xCA
...
# Test with all real controller packets
test_packets = [
    (bytes.fromhex("CA 35 02 00 00 00 00 00 00 00 00 00 00 00 00"), bytes.fromhex("9B A0"), "Alive"),
    (bytes.fromhex("CA 35 00 00 00 01 01 00 00 FF 00 00 FF 00 00"), bytes.fromhex("99 7C"), "Data 1"),
    (bytes.fromhex("CA 35 00 03 00 01 FF 00 00 FF 00 00 FF 00 00"), bytes.fromhex("D6 90"), "Data 2"),
    (bytes.fromhex("CA 35 00 01 00 01 FF 00 00 FF 00 00 FF 00 00"), bytes.fromhex("AF 4D"), "Data 3"),
    (bytes.fromhex("CA 35 00 00 00 00 01 00 00 02 00 00 03 00 00"), bytes.fromhex("6F 31"), "Data 4"),
]

print("=" * 80)
print("TESTING CRC WITH HEADER INIT (User's Discovery!)")
print("=" * 80)
print("\nAlgorithm: crc1=0xCA, crc0=0x35, process all 15 bytes\n")

all_match = True

for full_packet, expected_crc, desc in test_packets:
    # Calculate CRC over all 15 bytes (including header)
    calc_crc = xkop_crc_with_header_init(full_packet)
    ...
    print(f"{desc:10} | Packet: {full_packet.hex().upper()}")
    print(f"           | Expected: {expected_crc.hex().upper()} | Calculated: {calc_crc.hex().upper()} | {status}")
    print()

print("=" * 80)
if all_match:
    print("🎉 SUCCESS! ALL PACKETS MATCH!")
    print("=" * 80)
    print("\nThe correct algorithm is:")
    print("  crc1 = 0xCA (header byte 1)")
    print("  crc0 = 0x35 (header byte 2)")
    print("  Process all 15 bytes including header")
    print("  Return [crc1, crc0]")
else:
    print("❌ Some packets don't match")
